                      ...
                      <div className="space-y-2">
                        {recentHistory && recentHistory.length > 0 ? (
                          recentHistory.map(item => {
                            ...
                            return (
                              <Link
                                ...
                                key={item.id}
                                className="flex justify-between items-center p-3 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral/20 transition-colors border border-transparent hover:border-gray-200 dark:hover:border-white/10"
                              >
                                <div className="flex items-center gap-3 overflow-hidden">
                                  {item.inputType === 'image'
                                    ? <FileImage size={18} className="text-gray-400 dark:text-white/60 flex-shrink-0"/>
                                    : <FileText size={18} className="text-gray-400 dark:text-white/60 flex-shrink-0"/>
                                  }
                                  <div className="overflow-hidden">
                                    <p className="font-medium text-gray-800 dark:text-white text-sm truncate max-w-xs">
                                      {displayText}
                                    </p>
                                    <p className="text-xs text-gray-400 dark:text-white/50">{dateLabel}</p>
                                  </div>
                                </div>
                                <span className={`text-xs font-bold px-3 py-1 rounded-full flex-shrink-0 ml-2 ${
                                  item.result?.isLikelyFake
                                    ? 'text-red-600 dark:text-danger-text bg-red-100 dark:bg-danger/10'
                                    : 'text-green-600 dark:text-success-text bg-green-100 dark:bg-success/10'
                                }`}>
                          ...
                          <div className="text-center py-8">
                            <p className="text-sm text-gray-400 dark:text-white/60">No recent activity to display.</p>
                          </div>
                        )}
                      </div>
                    </div>
