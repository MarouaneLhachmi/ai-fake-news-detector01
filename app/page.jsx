...
// Fonction pour convertir le texte Base64 (venant de l'upload) en vrai Fichier (Blob)
...
const startAnalysis = async (apiEndpoint, body, displayValue = null) => {
    setIsLoading(true);
    setError(null);
    setCurrentAnalysis(null);

    try {
      // Si c'est un FormData (un vrai fichier), on ne met PAS de header Content-Type manuellement
      ...
      const inputValue = displayValue || body.imageUrl || body.text || body.url;
  ...
  const handleImageUpload = async (url) => {
    // 'url' contient ici soit une URL web, soit l'image en Base64 (data:image...)
    
    if (url.startsWith("data:")) {
      // Cas 1 : C'est un FICHIER uploadé (Base64) -> On le convertit en FormData
      const blob = dataURLtoBlob(url);
      const formData = new FormData();
      formData.append("image", blob);
      
      // On envoie le FormData, et on passe l'URL 'url' juste pour l'affichage (3ème argument)
      await startAnalysis("/api/analyze-news", formData, url);
      
    } else {
      // Cas 2 : C'est une URL d'image (Image URL)
      // Note : Ton backend actuel semble exiger un fichier binaire ("image").
      // Si l'URL ne marche pas, c'est que le backend devra aussi être adapté pour télécharger l'URL.
      // Mais pour l'instant, on l'envoie comme avant (JSON) ou en FormData textuel selon besoin.
      // Essayons de l'envoyer proprement :
